...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket, // if the ticket's okay, just return the ticket
        Err(error) => { // if the new ticket produces an error, handle accordingly:
            if error.contains("Description") {
                Ticket::new(title, "Description not provided".to_string(), status).unwrap() // unwrap so that the Result type outputs a Ticket type
            } else {
                panic!("{error}");
            }
        }
